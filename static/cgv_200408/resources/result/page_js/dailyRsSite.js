...
//				fn_getDailyRSMSGrd();
				...
				$('.btn-tbltoggle').hide().eq(0).show();
				fn_getDailyRSMnInd();
//				fn_getDailyRSMnIndGrd();
			}
		}else if ($("[name='radio-rsSite']:checked").val() == "site"){
			if($el.attr("data-type") == "1"){
				fn_getDailySiteWtctm();
	...
	/* 토글버튼 이벤트 */
	$("#rs_type03 .btn-tbltoggle").on("click",function(){
		fn_toggleTh(this);
	});
	
	$("#site_type03 .btn-tbltoggle").on("click",function(){
		fn_toggleTh(this);
	});
}
...
//	console.log(e.target);
	...
	if($("[name='radio-rsSite']:checked").val() == "rs"){
		if(sortTable == "2"){
			fn_getDailyRSMSGrd(sortType,sortCol);
		}else if(sortTable == "3"){
			fn_getDailyRSMnIndGrd(sortType,sortCol);
		}
	}else if ($("[name='radio-rsSite']:checked").val() == "site"){
		if(sortTable == "1"){
			fn_getDailySiteWtctm(sortType,sortCol);
		}else if(sortTable == "2"){
			fn_getDailySiteMS(sortType,sortCol);
		}else if(sortTable == "3"){
			fn_getDailySiteMnInd(sortType,sortCol);
		}
	}
});
...
//전국대비 CGV 관람객 차트
function fn_getDailyRSWtctmWithBox(){
	var params = {srchDate:srchDate}
	transaction({
		url : '/result/getDailyRSWtctmWithBox.do'
		,params : params
	},function(result,e){
		if(!e){
			console.log("getDailyDyWtctm",result);
...
//RS별 CGV 관람객 차트
function fn_getDailyRSWtctm(){
	var params = {srchDate:srchDate}
	transaction({
		url : '/result/getDailyRSWtctm.do'
			,params : params
	},function(result,e){
		if(!e){
			console.log("fn_getDailyRSWtctm",result.rsWtctm);
...
//MS / BO MS 차트
function fn_getDailyRSMS(){
	var params = {srchDate:srchDate}
	transaction({
		url : '/result/getDailyRSMS.do'
		,params : params
		,complete : function(){}
	},function(result,e){
		if(!e){
			console.log("fn_getDailyRSMS",result);
...
//RS > MS > 하단그리드
function fn_getDailyRSMSGrd(orderType,orderCol){
	if(!orderType && !orderCol){
		orderType = "d";
		orderCol = "ms";
	}
	var params = {srchDate:srchDate , orderType:orderType , orderCol:orderCol};
	transaction({
		url : '/result/getDailyRSMSGrd.do'
		,params : params
	},function(result,e){
		if(!e){
			console.log("getDailyRSMSGrd",result);
...
//주요지표 차트
function fn_getDailyRSMnInd(){
	var params = {srchDate:srchDate}
	transaction({
		url : '/result/getDailyRSMnInd.do'
		,params : params
		,complete : function(){}
	},function(result,e){
		if(!e){
			console.log("fn_getDailyRSMnInd",result);
...
//RS > 주요지표 > 하단그리드
function fn_getDailyRSMnIndGrd(orderType,orderCol){
	if(!orderType && !orderCol){
		orderType = "d";
		orderCol = "atp";
	}
	var params = {srchDate:srchDate , orderType:orderType , orderCol:orderCol};
	transaction({
		url : '/result/getDailyRSMnIndGrd.do'
		,params : params
	},function(result,e){
		if(!e){
			console.log("getDailyRSMnIndGrd",result);
...
//사이트 > 관람객
function fn_getDailySiteWtctm(orderType , orderCol){
	if(!orderType && !orderCol){
		orderType = "d";
		orderCol = "cgvSpctCnt";
	}
	var params = {srchDate:srchDate , orderType:orderType , orderCol:orderCol};
	transaction({
		url : '/result/getDailySiteWtctm.do'
		,params : params
	},function(result,e){
		if(!e){
			console.log("fn_getDailySiteWtctm",result);
...
//사이트 > MS
function fn_getDailySiteMS(orderType,orderCol){
	if(!orderType && !orderCol){
		orderType = "d";
		orderCol = "ms";
	}
	var params = {srchDate:srchDate , orderType:orderType , orderCol:orderCol};
	transaction({
		url : '/result/getDailySiteMS.do'
		,params : params
	},function(result,e){
		if(!e){
			console.log("fn_getDailySiteMS",result);
...
//사이트 > 주요지표
function fn_getDailySiteMnInd(orderType,orderCol){
	if(!orderType && !orderCol){
		orderType = "d";
		orderCol = "atp";
	}
	var params = {srchDate:srchDate , orderType:orderType , orderCol:orderCol};
	transaction({
		url : '/result/getDailySiteMnInd.do'
		,params : params
	},function(result,e){
		if(!e){
			console.log("fn_getDailySiteMnInd",result);
...
function fn_mkChart05(data,dataFlag){
	...
	for(var i in data){
		...
		if(!isEmpty){
			...
			if(data[i].rcdTyp === "ATP") {
				chartData.push(data[i]);
			} else if (data[i].rcdTyp === "SPP") {
				chartData.push(data[i]);
			}
		}
	}
...
//		];
	...
	var maxData = 0;
	for(var i in chartData){
		chartData[i].rcdNm = "　"+chartData[i].rcdNm+"　";
		if(maxData < chartData[i].recode ) maxData = chartData[i].recode;
	}
...
//		if(!data[i].recode && data[i].recode != 0) isEmpty = true;
		...
		if(!isEmpty){
			switch(data[i].rcdTyp){
				case "GRP_SPCT_PER" : data[i].color = COLOR_ARR[5]; break;
			}
			if(data[i].rcdTyp === "GRP_SPCT_PER") {
				chartData.push(data[i]);
			}
		}
	}
	if(isEmpty){
		$("#chart06").css("padding-top","12px");
		$("#chart06").empty().append(ERR_HTML_TYPE02);
		return;
	}else{
		$("#chart06").css("padding-top","0");
	}
...
//			console.log("===========",data);
			...
			for(var i in data){
				if($("[name='radio-rsSite']:checked").val() == "site"){
					fnTxt = "onclick='fn_goDailySmry("+JSON.stringify(data[i])+")'";
					fnCnt = (Number(i)+1)+". ";
				}
				html += "<tr "+ fnTxt +" >";
				html += "<td data-cd='"+data[i].thatCd+"'>"+ fnCnt +gfn_removeCGVTxt( data[i].thatNm )+"</td>"; /*사이트*/
...
//			if(dataCode == "rsCd") fnTxt = "onclick=fn_getRsDtlPop('"+data[i][dataCode]+"')";
			html += "<tr "+ fnTxt +" >";
			html += "<td class='align-l' data-cd='"+data[i][dataCode]+"'>"+ fnCnt +gfn_removeCGVTxt( data[i][dataNm] )+"</td>"; /*SITE,RS*/
...
function fn_setRank(data,colNm,dataFlag){
	...
	for(var i=0; i<3; i++){
		var rank = {}
		rank.thatNm = gfn_removeCGVTxt(data[i].thatNm);
...
//			var a = data[i].atp == "-" ? 0 : data[i].atp;
//			var s = data[i].spp == "-" ? 0 : data[i].spp;
//			html += "<td>"+gfn_numberFormat( data[i].atpSpp )      +"</td>"; /*SPP실적*/
			html += "<td>"										+ fn_getPercentFormat(data[i].grpSpctPer)			+ "<br>";
			if(data[i].grpSpctYearDiffPer == "-"){ /*계획비차*/
				html += "<span>-</span>";
			}else if(data[i].grpSpctYearDiffPer < 0){
				html += "<span class='s-font fall'>"+ fn_getPercentFormat(Math.abs(data[i].grpSpctYearDiffPer)) +"</span></td>";
			}else if(data[i].grpSpctYearDiffPer > 0){
