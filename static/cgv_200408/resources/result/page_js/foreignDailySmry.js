...
/* 차트 데이터 조회 */
function fn_getForeignDailyChart(){
	var params = {}
	params.srchDate = $("#srchDate").val().replace(/-/gi,"");
	
	transaction({
		url : '/result/getForeignDailyChart.do'
		,params : params
		,complete : function(){}
	},function(result,e){
		if(!e){
			//console.log("getForeignDailyChart",result);
			fn_mkChart01(result.resultList);
			fn_getForeignDailyGrd(); //그리드 데이터 조회
		}else{
			//console.log(e);
		}
	});
...
/* 그리드 데이터 조회 */
function fn_getForeignDailyGrd(){
	var params = {}
	params.srchDate = $("#srchDate").val().replace(/-/gi,"");
	
	transaction({
		url : '/result/getForeignDailyGrd.do'
		,params : params
	},function(result,e){
		if(!e){
			//console.log("getForeignDailyGrd",result);
			fn_mkFrzTbl(result.resultList,null,fn_getDateList());
		}else{
			//console.log(e);
		}
	});
...
//			if(cnfNum < 10) cnfNum = "0"+cnfNum;
	...
	var nowDay = $("#srchDate").val().replace(/-/gi,"").substr(6,2);
	...
	for(var i in data){
		...
		for(var key in data[i]){
			...
			if(key.indexOf("col") > -1){
				...
				var nowDt = $("#srchDate").val().replace(/-/gi,"").substr(6,2);
//				var nowYm = $("#srchDate").val().replace(/-/gi,"").substr(0,6);
//				var nowDate = new Date();
//				if(Number(dateStr) > Number(nowDate.format("dd"))-1 && Number(nowDate.format("yyyyMM")) == nowYm){
				if(Number(dateStr) > Number(nowDt)){
					gainFallClass = "";
...
function fn_mkFrzTblCol(weekList,type){
	...
	if(type == "grp"){
		...
		for(var i in weekList){
			html += `<th class='${weekList[i].color!=99? 'f-color0'+weekList[i].color: '' }'>${weekList[i].dateDsplStr}<br/>${weekList[i].day}</th>`
		}
	}
	return html;
}
