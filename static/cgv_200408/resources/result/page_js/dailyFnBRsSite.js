...
function fn_init(){
	...
	fn_getDailyFnBRsChart(); //F&B 차트+RS 조회
...
//		$(".dataTypeBtn").eq(0).addClass("active");
		...
		if(rsSiteFlag == "rs"){ // RS 인경우
			...
			$(".dv-table-result.type01").show();
			fn_getDailyFnBRsSiteGrd(); //F&B RSSITE 조회
		}
...
//		console.log(e.target);
		...
		if($("[name='radio-rsSite']:checked").val() == "rs"){
			fn_getDailyFnBRsSiteGrd(sortType,sortCol);
...
/* TRANSACTION S ****************************************************************************/
//F&B RS 차트+그리드 조회
function fn_getDailyFnBRsChart(){
	var params = {}
	params.srchDate = $("#srchDate").val().replace(/-/gi,"");
	
	transaction({
		url : '/result/getDailyFnBRsChart.do'
		,params : params
		,complete : function(){}
	},function(result,e){
		if(!e){
			console.log("getDailyFnBRsChart",result.resultList);
			...
			fn_getDailyFnBRsSiteGrd(); //RSSITE 조회
		}else{
			console.log(e);
		}
	});
...
//RSSITE 조회
function fn_getDailyFnBRsSiteGrd(orderType, orderCol){
...
//	params.srchValue = "";
	...
	transaction({
		url : '/result/getDailyFnBRsSiteGrd.do'
		,params : params
	},function(result,e){
		if(!e){
			console.log("getDailyFnBRsSiteGrd",result.resultList);
...
function fn_mkRow(rowData, orderCol, rsSiteFlag){
	...
	for(var i in rowData){
		if("site" == rsSiteFlag){
			html += `<tr>
						<td onclick='fn_goDailySmry(${JSON.stringify(rowData[i])})' data-cd='${rowData[i].cd}'>${gfn_removeCGVTxt(rowData[i].nm)}</td>
			`;
...
function fn_goDailySmry(obj){
	var url = "/result/dailyFnBSmry.do";
	url += "?pageId="+pageId;
	url += "&rsSiteType=site";
