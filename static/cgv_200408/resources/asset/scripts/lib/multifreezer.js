...
$('.table-freeze-multi').each(function () {
    ...
    console.log("cloneLeft",cloneLeft);
    ...
    $('.table-freeze-multi:eq(1) tbody th').text('').css('background', 'white').css('border', 0);
	$('.table-freeze-multi:eq(0) thead tr th:eq(0)').text('').css('background', 'white').css('border', 0);
	$('.table-freeze-multi:eq(0) thead tr th:eq(1)').text('').css('background', 'white').css('border', 0);
    /* 테이블 좌측고정 스크롤 (iscroll) */
    /*var width = $(".iscroll .freeze-multi-scroll-table-head-inner").width();
    $(".freeze-multi-scroll-table").css("width",width);
...
/*var osName, versionName;
if ( osVersion != 'undefined' && osVersion!=null && osVersion) {
	var osData = osVersion.split(":");
	osName = osData[0];
	versionName = osData[1].split(".")[0];
	if (osName == "I" && Number(versionName) >= 13){ //IOS 운영체제의 경우에만
		$(".freezer").addClass("iscroll");
	}
}*/
