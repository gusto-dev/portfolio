...
* anypicker 함수 (시간)
* ============================== */
var prevTm; //미소지기운영 변수
$.fn.timeAnyPicker = function(option){
  $(this).AnyPicker({
    mode: "select",
...
//       console.log("out",item);
      ...
      if(option){
//    	  console.log(prevTm,item.values[0]);
    	  if(option.minValue){
    		  var now = option.minValue;
    		  if(option.maxValue){
    			  if(!prevTm) prevTm = now;
        		  if( Number(item.values[0].val) < Number(prevTm.substr(0,2)) ){
        			  alert("검색기준보다 이전시간은 재설정 할 수 없습니다.");
        				  return option.maxValue;
        		  }else if( Number(item.values[0].val) == Number(prevTm.substr(0,2)) ){
        			  if( Number(item.values[1].val) < Number(prevTm.substr(3,2)) ){
        				  alert("검색기준보다 이전시간은 재설정 할 수 없습니다.");
    					  return option.maxValue;
        			  }
        		  }
        	  }else{
        		  if( Number(item.values[0].val) < Number(now.substr(0,2)) ){
        			  alert("현재시간보다 이전시간은 재설정 할 수 없습니다.");
    				  return now;
        		  }else if( Number(item.values[0].val) == Number(now.substr(0,2)) ){
        			  if( Number(item.values[1].val) < Number(now.substr(3,2)) ){
        				  alert("현재시간보다 이전시간은 재설정 할 수 없습니다.");
        				  return now;
        			  }
        		  }else{
        			  prevTm = item.values[0].val+":"+ item.values[1].val;
        		  }
        	  }
    	  }
      }
