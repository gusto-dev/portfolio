...
$(function ($) {
	 ...
	 * 테이블 아코디언
	 * ============================== */
	$(".accordion-type .btn-accordion").attr("status", "c");
	$(".accordion-type .btn-accordion").click(function () {
		var this_el = $(this);
		var target = $(this).parents(".group-top").next();
		if ($(this_el).attr("status") == "c") fn_tbl_show(target, this_el);
		else fn_tbl_hide(target, this_el);
	});
	/* ==============================
	 * 테이블 아코디언 전체 open 버튼
	 * ============================== */
	$(".btn-round.full-view").click(function () {
		var this_el = $(this);
		var target = $(this).parents().find(".group-bottom");
		fn_tbl_show(target, this_el);
		$(".accordion-type .btn-accordion").attr("status", "o");
	})
	...
	$(".scroll-type").scroll(function () {
		...
		if (scroll > 0) {
			$(this).find(".icon-x-scroll").hide();
		} else {
			$(this).find(".icon-x-scroll").show();
		}
...
const COLOR_ARR_02 = ["#d1d7e0", "#67b7dc", "#6893dd", "#6670db", "#8067dc", "#a78bd4", "#c567db", "#dc67cf", "#fd82aa"]; //첫번째 컬러 회색
...
function fn_getFlagDate(type, noSepFlag) {
	...
	var defaultDate;
	if (type == "d") {
		defaultDate = new Date(y, m, d).format("yyyy-MM-dd");
		if (noSepFlag) new Date(y, m, d).format("yyyyMMdd");
	} else {
		defaultDate = new Date(y, m).format("yyyy-MM");
		if (noSepFlag) new Date(y, m, d).format("yyyyMM");
	}
